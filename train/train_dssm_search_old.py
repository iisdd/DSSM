...
sparse_item_feature = ['s_i_c2b_evaluate_score_segment', 's_i_city_id', 's_i_emission_standard', 's_i_minor_category_id', 's_i_model_price_bin', 's_i_car_year', 's_i_gearbox', 's_i_air_displacement', 's_i_tag_id', 's_i_car_color', 's_i_guobie', 's_i_c2b_evaluate_level', 's_i_auto_type', 's_i_fuel_type', 's_i_seats', 's_i_c2b_ctob_car_level', 'clue_id']
...
def dssm_model(samples_data):
    ...
    emb_dims = {key: int(v**0.5) + 1 for key, v in vocab_size.items()}
    ...
    return keras.models.Model(inputs=s_u_Inputs + d_u_Inputs + s_i_Inputs + d_i_Inputs, outputs=output), \
        keras.models.Model(inputs=s_u_Inputs + d_u_Inputs, outputs=user_vector), \
...
model_save_path = f'/home/jupyterhub/daiyuxuan/dssm_models_search/model_file/{today_date}_old/'
...
x_train = [train_data[fea] for fea in total_fea]  # 变成数组
y_train = train_data['label']
x_valid = [valid_data[fea] for fea in total_fea]
y_valid = valid_data['label']
x_test = [test_data[fea] for fea in total_fea]
